...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  if (action === 'suspend') {
    await db.user.update({ where: { id: params.id }, data: { status: 'SUSPENDED' } })
  ...
  if (action === 'delete') {
    await db.user.update({ where: { id: params.id }, data: { status: 'DELETED' } })
    return NextResponse.json({ ok: true })
  }
  ...
  if (action === 'change_plan' && planId) {
    ...
    await db.subscription.upsert({
      ...
      create: { userId: params.id, planId, status: 'ACTIVE', startedAt: new Date() },
